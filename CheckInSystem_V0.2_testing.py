...
# 定义一个函数来显示预测结果
def draw_predict(frame, label):
    # 定义一个字典来存储学号和姓名的对应关系
    names = {1: "farid", 2: "roice"}
    # 在图像上绘制标签和姓名
    # cv2.putText(frame, str(label), (x + 5, y - 5), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)
    # cv2.putText(frame, names[label], (x + 5, y + h - 5), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)


# 读取已有的人脸照片和学号
print("正在读取照片~")
images, labels = read_images_and_labels("faces")
...
# 创建标签对象，显示提示信息
label_msg = tk.Label(window, text="欢迎使用打卡程序！", font=("Arial", 16))
# 将标签对象放置在窗口中
label_msg.pack()
